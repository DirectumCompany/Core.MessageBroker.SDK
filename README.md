...
* **InvalidCredentialTypeError** - указан неверный типа реквизита получателя. Примеры сообщений об ошибке:
  * "Credential type '{credentialType}' in {messageId} message is invalid for {pluginType} proxy".
...
* **TransmitRetryExceededError** - ошибка при слишком большом количестве попыток отправки. Не используется напрямую в плагинах. Работает в паре с настройкой `MaxTransmitRetryCount`. Примеры сообщений об ошибке:
  * "Too many tries to send message {messageId} by {deliveryProxyName} proxy".
...
Коды ошибок, при которых попытки отправить конкретное сообщение будут через некоторый промежуток (повторная попытка будет через `ProcessDelayMs` мс):
