...
* `bool` **IsSuccess** - `true`, если сообщение было успешно отправлено. Устаревшее свойство, оставленное для сохранения обратной совместимости. Рекомендуется использовать **Status** для определения статуса операции; 
* `TransmitStatus` **Status** - cтатус отправки сообщения:
  * `enum TransmitStatus`:
    * `Delivered = 0` - доставлено;
    * `Sent = 1` - передано, доставка зависит от провайдера. Для случаев, когда провайдер принял запрос на отправку сообщения, но сама доставка может быть отложена или маловероятна. Может быть применимо для чатов, когда пользователь давно не заходил в мобильное приложение;
    * `Failed = 2` - отправка не удалась;
